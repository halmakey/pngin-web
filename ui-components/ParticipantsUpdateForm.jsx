...
export default function ParticipantsUpdateForm(props) {
  const {
    id: idProp,
    participants,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = participantsRecord
      ? { ...initialValues, ...participantsRecord }
      : initialValues;
    setCreatedAt(cleanValues.createdAt);
    setUpdatedAt(cleanValues.updatedAt);
    setDiscordId(cleanValues.discordId);
    setDiscordName(cleanValues.discordName);
    setRole(cleanValues.role ?? []);
    setCurrentRoleValue(undefined);
    setFile(cleanValues.file);
    setErrors({});
  };
  const [participantsRecord, setParticipantsRecord] =
    React.useState(participants);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Participants, idProp)
        : participants;
      setParticipantsRecord(record);
    };
    queryData();
  }, [idProp, participants]);
  React.useEffect(resetStateValues, [participantsRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Participants.copyOf(participantsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ParticipantsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Created at"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || participants)}
